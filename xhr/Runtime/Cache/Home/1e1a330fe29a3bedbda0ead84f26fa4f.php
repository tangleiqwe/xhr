...
<div class="mui-content">
    <div class="mui-card">
    <h5 class="mui-content-padded">操作方式：<br><br>1、提交提现申请。<br><br>
        2、平台进行审核转币（工作时间2个小时内完成）。<br>
    </h5>
    <form class="mui-input-group">
        <div class="mui-input-row">
            <h5 ><label style="width: 30%;">玩客币余额：</label><label style="width:70%;" id="zh"><?=number_format($pnum,2)?></label>
            </h5>fz
        </div>
    </form>
</div>
    <div class="mui-card">
    <h5 class="mui-content-padded">π提现信息</h5>
    <form class="mui-input-group"  action="/paipai/index.php/Home/My/paitxinsert" method="post">
        <div class="mui-input-row">
            <label style="width: 30%;">链克账户</label><label style="width:70%;" id="paizh"> <?=$mjzh?></label>
        </div>
        <div class="mui-input-row">
            <label>提现数量</label>
            <input id='txsl'name="txsl" type="number" class="mui-input-clear mui-input" placeholder="请输提现数量，最少5个">
        </div>
        <div class="mui-card-footer">
        <button id='login' type="submit" class="mui-btn mui-btn-block mui-btn-primary"  >提交</button>
            </div>
    </form>
    <!--<h5 class="mui-content-padded">可撤销的π提现信息</h5>
        ...
        <div  class="mui-content ">
            <div id="scroll" class="mui-scroll-wrapper">
                <div class="mui-scroll">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a onclick="paitx_del(<?=$vo['id']?>)">
                            <div class="mui-card-footer" style="padding-top: 0px;padding-bottom: 0px;padding-left: 0px;padding-right: 0px">
                                <ul class="mui-table-view mui-grid-view mui-grid-12" >
                                    ...
                                    <li class="mui-table-view-cell mui-media mui-col-xs-3 mui-col-sm-3" >
                                        <span class="" style="font-size: 12px"><?php echo ($vo["pnum"]); ?></span>
                                    </li>
                                    <li class="mui-table-view-cell mui-media mui-col-xs-3 mui-col-sm-3" >
...
<script>
    function paitx_del(id) {
        var btnArray = ['取消', '确定'];

        mui.confirm('您确定要撤销此记录？', '操作提示', btnArray, function(e) {
            if (e.index == 1) {
                    window.location.href = "/paipai/index.php/Home/My/paitxdel/id/"+ id ;
            } else {
            }
        });
    }
