...
<div class="mui-content">
    <div>
        <div id="segmentedControl" class="mui-segmented-control mui-segmented-control-inverted mui-segmented-control-negative">
            <?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a class="mui-control-item <?php if(empty($tab)){ if($i == 1) {echo 'mui-active';} } else { if($vo['id']== $tab) echo 'mui-active';}?>" href="#item<?php echo ($vo["id"]); ?>">
                    <?php echo ($vo["name"]); ?>
                </a><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
    </div>
    <div>
        <?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div id="item<?php echo ($vo["id"]); ?>" class="mui-control-content <?php if($vo['id'] == $tab) { echo 'mui-active'; } ?>"  style="height: 500px;margin-bottom: 80px">
                <div id="scrol<?php echo ($vo["id"]); ?>" class="mui-scroll-wrapper">
 ...
 $list = M('videolist')->where('v_type = '.$vo['id'])->order('id desc')->limit(9)->select(); ?>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_p): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell mui-media mui-col-xs-4" >
                                    <a id="<?php echo ($vo_p["id"]); ?>" href="/xhr/index.php/Home/Index/video/id/<?php echo ($vo_p["id"]); ?>">
...
<script>
    ...
    function addendList(obj) {
        ...
        lis = ul.children();
        $.ajax({
            url:"/xhr/index.php/Home/Index/new_tab_list",
